...
/**
 * Cache that replaces the least recently used cached item.
 ...
 */
class SynchronizedLRUCache[K, V](override val maxSize: Int)(f: K => V)
  ...
  override protected def expire(store: ConcurrentHashMap[K, Option[V]]): Option[K] = {
    if (!isMaxedOut()) { None }
      ...
      val key = expirationOrder.remove(0)
      store.remove(key)
      Some(key)
    }
  }
...
object SynchronizedLRUCache {
  def getInstance[K, V](maxSize: Int)(f: K => V) =
    new SynchronizedLRUCache[K, V](maxSize)(f)
}